...
struct LocationShared {
    ...
    void push_multi_pci_history(const std::map<int, CellSample>& cellsByPci) {
        ...
        for (const auto& [pci, _] : cellsByPci) {
            ...
            if (hist_rsrp_by_pci.find(pci) == hist_rsrp_by_pci.end()) {
                hist_rsrp_by_pci[pci] = std::vector<double>(hist_x.size() - 1, 0.0);
            }
            if (hist_rssi_by_pci.find(pci) == hist_rssi_by_pci.end()) {
                hist_rssi_by_pci[pci] = std::vector<double>(hist_x.size() - 1, 0.0);
            }
            if (hist_sinr_by_pci.find(pci) == hist_sinr_by_pci.end()) {
                hist_sinr_by_pci[pci] = std::vector<double>(hist_x.size() - 1, 0.0);
            }
        }

        for (auto& [pci, values] : hist_rsrp_by_pci) {
            auto it = cellsByPci.find(pci);
            if (it != cellsByPci.end() && it->second.has_rsrp) values.push_back(it->second.rsrp);
            else values.push_back(0.0);
        }

        for (auto& [pci, values] : hist_rssi_by_pci) {
            auto it = cellsByPci.find(pci);
            if (it != cellsByPci.end() && it->second.has_rssi) values.push_back(it->second.rssi);
            else values.push_back(0.0);
        }

        for (auto& [pci, values] : hist_sinr_by_pci) {
            auto it = cellsByPci.find(pci);
            if (it != cellsByPci.end() && it->second.has_sinr) values.push_back(it->second.sinr);
            else values.push_back(0.0);
        }
    ...
    void trim_history() {
        if (hist_x.size() <= MAX_HISTORY) return;

        hist_x.erase(hist_x.begin());

        for (auto& [_, values] : hist_rsrp_by_pci) {
            if (!values.empty()) values.erase(values.begin());
        }
