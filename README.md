# Пастухов Александр Андреевич

**Группа:** ИКС-432

---

# Backend-сервер (ПР8)

Данный репозиторий создан в рамках **Практических работ №8-10** и содержит примеры клиент-серверного взаимодействия по протоколу **TCP** с использованием языков **Python**, **C** и Android-клиента (submodule).  

---

## Цель работы

- Изучить основы клиент-серверного взаимодействия
- Реализовать передачу данных по TCP
- Проверить обмен данными между программами на разных языках
- Проанализировать сетевой трафик с помощью **Wireshark**

---

## Структура репозитория
backend-server/  
├─ android-client/ # Android-приложение (submodule)  
├─ examples/  
│ ├─ server.py # TCP-сервер на Python  
│ ├─ client.py # TCP-клиент на Python  
│ ├─ server.c # TCP-сервер на C  
│ └─ client.c # TCP-клиент на C  
└─ README.md  

---

## Описание примеров

### server.py

TCP-сервер, который: слушает порт **25565**, принимает входящее соединение, получает сообщение от клиента, отправляет ответ `"Hello Client"`.  

Используемые функции: `socket()`, `bind()`, `listen()`, `accept()`, `recv()`, `send()`

---

### client.py

TCP-клиент, который: подключается к `localhost:25565`, отправляет строку `"Hello, server!"`, получает ответ от сервера, выводит полученные данные в консоль.  

---

### server.c

TCP-сервер на языке C: создаёт сокет, привязывается к порту **25565**, принимает одно соединение, получает сообщение от клиента, отправляет ответ `"Hello from c-server"`.

Используются системные вызовы Berkeley sockets: `socket`, `bind`, `listen`, `accept`, `read` ,`send`.

---

### client.c

TCP-клиент на языке C: подключается к `127.0.0.1:25565`, отправляет сообщение `"Hello from client"`, принимает ответ сервера, выводит результат в терминал.

---

## Взаимодействие между Python и C

Реализованы следующие сценарии:
- **Python client - Python server**
- **C client - Python server**
- **Python client - C server**
- **C client - C server**

Это подтверждает совместимость TCP-протокола между программами, написанными на разных языках.

---

## Анализ трафика (Wireshark)

При запуске клиента и сервера:
- в Wireshark фиксируются TCP-пакеты
- видны передаваемые строки: `"Hello, server!"`, `"Hello Client"`, `"Hello from client"`, `"Hello from c-server"`

Используется фильтр: tcp.port == 25565

---

## ПР9  
### Работа с сокетами (ZMQ). Передача данных от Android к PC

В рамках данной практической работы реализована передача данных между Android-приложением и серверным приложением на ПК с использованием библиотеки **ZeroMQ (ZMQ)**.

---

## Цель работы

- Изучить работу с сокетами ZeroMQ
- Реализовать клиент-серверное взаимодействие Android ↔ PC
- Настроить передачу данных по TCP
- Закрепить работу с потоками в Android
- Освоить работу с ветками Git и merge request

---

## Общая схема взаимодействия  

[ Android-приложение ]  
|  
| ZMQ (REQ)  
| "Hello from Android!"  
v  
[ Python-сервер (PC) ]  
|  
| ZMQ (REP)  
| "Hello from Server!"  
v  
[ Консоль + файл логов ]  

---

## Серверная часть (PC)

Серверная часть запускается на компьютере и реализована на языке **Python** с использованием ZeroMQ.

Функционал сервера:
- приём сообщений от Android-клиента;
- отправка ответного сообщения `"Hello from Server!"`;
- сохранение каждого принятого сообщения в файл;
- подсчёт количества полученных пакетов;
- вывод всех сохранённых данных в консоль.

---

## Условия выполнения

- Android-устройство и компьютер находятся в одной сети
- Сервер запускается до подключения клиента
- Передача данных инициируется нажатием кнопки в Android-приложении

---

## ПР10  
### Сериализация. Data-классы. Формирование JSON

В рамках практической работы №10 реализовано формирование структуры данных (DTO) на Android, сериализация в JSON и периодическая отправка на сервер по **ZeroMQ** (TCP). На стороне сервера данные принимаются, выводятся в консоль и сохраняются в файл.  

---

## 6.1 Схема архитектуры программного комплекса (от ПР6 до ПР10)

Начиная с получения Location (ПР6) и заканчивая передачей JSON-пакетов (ПР10):

[ Android ]  
├─ LocationActivity (ПР6) -> получение координат  
├─ TelephonyActivity (ПР7) -> получение информации о сотах (GSM/LTE/NR)  
├─ TelemetryBuilder (ПР10) -> сбор DTO -> JSON  
└─ SerializationActivity (ПР10) -> отправка JSON каждые 1s (ZMQ REQ)  
|  
| tcp://<SERVER_IP>:5555  
v  
[ Desktop Server (Python + ZMQ) ]  
├─ ZMQ REP socket -> прием JSON  
├─ вывод в консоль  
├─ счетчик пакетов  
└─ сохранение каждого пакета в файл (лог)  

---

## 6.2 Описание архитектуры клиента (Android)

### Структура проекта (файловая система)

Клиентская часть реализована в виде Android-приложения на языке Kotlin.  
Основные компоненты, задействованные в ПР10:  
app/  
├─ java/com/example/calculator/  
│ ├─ SerializationActivity.kt  
│ ├─ TelemetryBuilder.kt  
│ ├─ TelemetryDtos.kt  
│ ├─ LocationActivity.kt  
│ └─ TelephonyActivity.kt  
└─ res/layout/  
└─ activity_serialization.xml  

- `SerializationActivity.kt` - управление процессом передачи данных (Start / Stop);  
- `TelemetryBuilder.kt` - сбор данных Location и CellInfo, формирование DTO и JSON;  
- `TelemetryDtos.kt` - data-классы для сериализации;  
- `LocationActivity.kt` - получение координат устройства;  
- `TelephonyActivity.kt` - получение данных о сотовых сетях;  
- `activity_serialization.xml` - пользовательский интерфейс для запуска передачи.  

---

### Архитектура с точки зрения потоков (Thread)

В приложении используются два типа потоков:

- **UI-поток**
  - обработка нажатий кнопок;
  - отображение состояния передачи;
  - вывод информации пользователю.

- **Фоновый поток**
  - запускается при нажатии кнопки Start;
  - реализован через `Thread { sendLoop() }`;
  - с периодичностью **1 секунда**:
    - формирует структуру DTO;
    - сериализует данные в JSON;
    - отправляет данные на сервер;
    - получает ответ от сервера.

Использование фонового потока предотвращает блокировку UI.

---

### Endpoint соединения с сервером

Для передачи данных используется ZeroMQ.

- Протокол: **TCP**
- Тип сокета: **REQ**
- Endpoint сервера: tcp://<SERVER_IP>:5555

---

## 6.3 Описание архитектуры сервера (Python)

### Структура проекта (файловая система)

Серверная часть реализована на языке Python с использованием ZeroMQ.  
backend-server/  
├─ examples/  
│ └─ server_zmq.py  
└─ README.md  

- `server_zmq.py` - сервер ZeroMQ, принимающий данные от Android-клиента.

---

### Архитектура с точки зрения потоков (Thread)

Сервер реализован в виде однопоточного приложения:

- создаётся ZMQ-сокет типа **REP**;
- сервер постоянно ожидает входящие сообщения;
- при получении сообщения:
  - данные выводятся в консоль;
  - увеличивается счётчик принятых пакетов;
  - данные сохраняются в файл (лог).

---

### Endpoint’ы сервера

Сервер прослушивает входящие подключения по адресу: tcp://0.0.0.0:5555  

Используемый тип сокета:
- **REP** - сервер отвечает каждому клиентскому запросу.

---

# ПР13  
## Backend + PostgreSQL + Визуализация сигналов

---

## Цель работы

Доработать серверное приложение для:
- сохранения телеметрии в базу данных PostgreSQL;
- обработки входящих данных от Android-приложения;
- отображения текущих параметров в GUI;
- построения графиков уровня сигнала для нескольких сот.

---

## Общая архитектура

[ Android-приложение ]  
├─ сбор данных (Location + Telephony)  
├─ формирование JSON (Telemetry)  
└─ отправка по ZMQ (REQ)  

| tcp://<SERVER_IP>:5555  

[ C++ Backend Server ]  
├─ ZMQ REP socket  
├─ парсинг JSON  
├─ обновление GUI (текущие значения)  
├─ построение графиков (ImGui + ImPlot)  
└─ запись в PostgreSQL  

|  

[ PostgreSQL ]  
├─ telemetry_packets  
└─ telemetry_cells  

---

## Структура базы данных

### Таблица telemetry_packets

Хранит основные данные пакета:

```sql
CREATE TABLE telemetry_packets (
    id SERIAL PRIMARY KEY,
    ts_client_ms BIGINT,
    latitude DOUBLE PRECISION,
    longitude DOUBLE PRECISION,
    altitude DOUBLE PRECISION,
    accuracy DOUBLE PRECISION,
    provider TEXT
);```

### Таблица telemetry_cells

Хранит данные по каждой соте (PCI):

```sql
CREATE TABLE telemetry_cells (
    id SERIAL PRIMARY KEY,
    packet_id INTEGER REFERENCES telemetry_packets(id),
    radio TEXT,
    pci INTEGER,
    rsrp DOUBLE PRECISION,
    rsrq DOUBLE PRECISION,
    rssi DOUBLE PRECISION,
    sinr DOUBLE PRECISION,
    asu INTEGER
);```

---

## Логика работы сервера  
  
При получении каждого JSON-пакета:  
	1. Парсится блок данных (location, traffic, cells);  
	2. В таблицу telemetry_packets добавляется запись;  
	3. Получается packet_id;  
	4. Для каждой соты:  
		1. извлекаются параметры (PCI, RSRP, RSSI, SINR и др.);  
		2. создаётся запись в telemetry_cells.  

---

## Работа GUI  
  
GUI построен на Dear ImGui + ImPlot.  
  
### Отображаемые данные:  
	1. текущая локация (lat, lon, alt);  
	2. параметры сигнала;  
	3. JSON последнего пакета;  
	4. информация о сети.  
  
---
  
## Графики сигналов  
  
Реализованы графики:  
	1. RSRP;  
	2. RSSI;  
	3. SINR;  

---

Принцип построения графиков  
	1. при поступлении новых данных значения добавляются в массивы;  
	2. для каждого PCI ведётся отдельная история;  
	3. по оси X - номер измерения (Sample);  
	4. по оси Y - уровень сигнала.  

---

## Поддержка нескольких сот  
  
Если в одном пакете приходят несколько сот:  

PCI 7 - RSRP = -100  
PCI 13 - RSRP = -89  
  
то:  
	1. на графике отображаются несколько линий одновременно;  
	2. каждая линия соответствует своему PCI;  
	3. линии отображаются разными цветами;  
	4. значения отображаются параллельно по одной оси X.  

---

## Легенда графика  

	1. в легенде отображаются все обнаруженные PCI;  
	2. каждый PCI соответствует своей линии;  
	3. позволяет отслеживать поведение каждой соты во времени.  

---

## Особенности реализации

	1. графики строятся в реальном времени;  
	2. данные берутся из оперативной памяти (не из БД);  
	3. БД используется только для хранения;  
	4. реализована фильтрация некорректных значений (например, 2147483647);  
	5. поддерживается работа с несколькими типами сетей (GSM, LTE, NR).  

---

# ПР14  
## Backend + OpenStreetMap

---

## Цель работы

Доработать backend-сервер для:

- отображения карты OpenStreetMap в GUI;
- загрузки и кеширования тайлов;
- пересчёта координат (lat/lon) в координаты тайлов;
- динамического отображения карты в зависимости от размера окна;
- адаптации карты под изменение координат и zoom.

---

## Общая архитектура

[ Android-приложение ]  
├─ получение координат (Location)  
└─ отправка JSON (ZMQ REQ)  

| tcp://<SERVER_IP>:5555  

[ C++ Backend Server ]  
├─ ZMQ REP socket  
├─ парсинг JSON  
├─ обновление текущих координат  
├─ TileManager (работа с тайлами)  
├─ загрузка тайлов (libcurl)  
├─ декодирование PNG (stb_image)  
└─ отображение карты (ImGui + ImPlot)  

|  

[ Локальное хранилище ]  
build/  
└─ <zoom>/<x>/<y>.png  

---

## Установка зависимостей

Работа выполняется в среде **MSYS2 (MINGW64)**.

Установка необходимых библиотек:

```bash
pacman -S --needed mingw-w64-x86_64-toolchain
pacman -S --needed mingw-w64-x86_64-glfw
pacman -S --needed mingw-w64-x86_64-zeromq
pacman -S --needed mingw-w64-x86_64-curl
pacman -S --needed mingw-w64-x86_64-postgresql
pacman -S --needed mingw-w64-x86_64-cmake
pacman -S --needed mingw-w64-x86_64-pkgconf
```

Дополнительно требуется:  
	1. библиотека stb_image.h (подключается вручную в проект)  
	2. Dear ImGui + ImPlot (подключены в репозитории)  

---

## 📌 4. Компиляция

---

## Компиляция

Выполняется из директории проекта:

```bash
mkdir build
cd build
cmake ..
cmake --build .
```

В результате будет создан исполняемый файл: server_app.exe  

---

## 📌 5. Запуск

---

## Запуск

```bash
./server_app.exe
```

После запуска открывается GUI-приложение, содержащее:
	1. окно текущей локации;  
	2. информацию о сети;  
	3. графики сигналов;  
	4. окно карты OpenStreetMap (OSM Map).  

Для корректной работы:
	1. Android-устройство должно быть подключено к той же сети;  
	2. сервер должен быть запущен до клиента;  
	3. координаты должны поступать в реальном времени.  

---

## 📌 6. Работа с тайлами OSM

---

## Работа с тайлами OpenStreetMap

### Пересчёт координат

Текущие координаты (latitude, longitude) преобразуются в координаты тайлов:

- `lon - tile_x`
- `lat - tile_y`

Используется схема тайлов OpenStreetMap.

---

### Загрузка тайлов

Если тайл отсутствует локально:

- формируется URL: https://tile.openstreetmap.org/{z}/{x}/{y}.png
- выполняется загрузка через **libcurl**;
- файл сохраняется в директорию: build/<zoom>/<x>/<y>.png  

---

### Кеширование

Если тайл уже существует:

- повторная загрузка не выполняется;
- используется локальный файл.

Это снижает нагрузку на сеть и ускоряет отображение карты.

---

## 7. Обработка изображений

---

## Обработка изображений

Для декодирования PNG используется библиотека:

- **stb_image.h**

Процесс:

1. загрузка PNG-файла;
2. преобразование в RGBA массив;
3. передача данных в OpenGL;
4. создание текстуры для отображения.

Каждый тайл хранится как OpenGL texture.

---

## 8. Отображение карты

---

## Отображение карты

Карта отображается в отдельном окне: OSM Map  

Используемые технологии:

- Dear ImGui
- ImPlot

---

### Принцип работы

- карта строится вокруг текущих координат;
- определяется центральный тайл;
- загружается сетка соседних тайлов;
- тайлы отображаются как изображения (`PlotImage`).

---

### Динамическое количество тайлов

Количество тайлов зависит от размера окна:

- размер тайла: 256x256 пикселей;
- при увеличении окна - загружается больше тайлов;
- при уменьшении - меньше.

---

### Zoom

Реализован слайдер zoom:

- влияет на уровень детализации карты;
- изменяет координаты тайлов;
- вызывает загрузку новых тайлов при необходимости.

---

## 9. Особенности реализации

---

## Особенности реализации

- карта обновляется при изменении координат;
- используется локальное кеширование тайлов;
- загрузка тайлов выполняется при необходимости;
- структура проекта разделена на модули:
  - `osm_math` - работа с координатами тайлов;
  - `curl_func` - загрузка файлов;
  - `tile_manager` - управление тайлами и текстурами;
  - `gui_thread` - интерфейс;
  - `server_thread` - обработка данных;
- реализована интеграция с существующей системой телеметрии.

---

## 10. Ограничения

---

## Ограничения

- загрузка тайлов выполняется синхронно (возможны задержки интерфейса);
- отсутствует ручное перемещение карты мышью;
- позиционирование карты зависит от текущих координат устройства;
- отсутствует предзагрузка тайлов.

---

## 11. Вывод

---

## Вывод

В рамках практической работы реализовано отображение карт OpenStreetMap в GUI-приложении.

Были освоены:

- работа с географическими координатами;
- загрузка данных по HTTP (libcurl);
- декодирование изображений;
- работа с графическим интерфейсом (ImGui + ImPlot);
- организация кеширования данных.

---

# ПР15  
## Backend + Heatmap

---

## Цель работы

Доработать backend-сервер для генерации и отображения тепловой карты качества сигнала мобильной сети.

В рамках практической работы реализовано:

- построение Heatmap на основе принятых GPS-точек;
- использование метода обратных взвешенных расстояний — IDW;
- отображение тепловой карты поверх OpenStreetMap;
- выбор критерия отображения в GUI;
- работа с параметрами RSRP, RSRQ, RSSI и Altitude;
- построение карты отдельно для разных EARFCN;
- ограничение радиуса интерполяции;
- сохранение сгенерированных изображений heatmap.

---

## Назначение Heatmap

Heatmap используется для визуальной оценки качества мобильного сигнала на карте.

Каждая точка измерения содержит:

- координаты устройства;
- высоту;
- PCI;
- EARFCN;
- RSRP;
- RSRQ;
- RSSI;
- SINR.

На основе этих данных backend строит цветовую карту покрытия.

---

## Поддерживаемые критерии

В интерфейсе можно выбрать параметр, по которому строится тепловая карта:

- `RSRP` — уровень опорного сигнала LTE;
- `RSRQ` — качество принимаемого сигнала;
- `RSSI` — общий уровень принимаемого сигнала;
- `Altitude` — высота устройства.

Для каждого EARFCN тепловая карта может строиться отдельно.

---

## Метод IDW

Для генерации Heatmap используется метод обратных взвешенных расстояний:

**IDW — Inverse Distance Weighting**

Суть метода:

- чем ближе экспериментальная точка к вычисляемой точке, тем сильнее её влияние;
- чем дальше точка, тем меньше её вес;
- итоговое значение считается как взвешенное среднее.

---

## Формула IDW

```text
V = Σ(value_i * weight_i) / Σ(weight_i)
```

где:

- `V` — итоговое значение в вычисляемой точке;
- `value_i` — значение сигнала в экспериментальной точке;
- `weight_i` — вес точки.

Вес считается по формуле:

```text
weight_i = 1 / distance_i^p
```

где:

- `distance_i` — расстояние до экспериментальной точки;
- `p` — степень влияния расстояния.

Обычно используется значение:

```text
p = 2
```

---

## Ограничение радиуса интерполяции

Чтобы тепловая карта не строилась по слишком далёким точкам, используется ограничение радиуса.

Учитываются только точки, находящиеся в пределах:

```text
10 - 40 метров
```

Если рядом с вычисляемой точкой нет экспериментальных измерений, область не закрашивается.

Это позволяет избежать ложной интерполяции там, где реальные измерения отсутствуют.

---

## Критерии качества RSRP

Для параметра RSRP используется следующая шкала:

| Уровень сигнала | Значение | Описание |
|---|---|---|
| Excellent | > -80 dBm | сильный сигнал |
| Good | -80 ... -90 dBm | хороший сигнал |
| Fair / Marginal | -90 ... -100 dBm | средний сигнал |
| Poor / Weak | -100 ... -110 dBm | слабый сигнал |
| No Signal | < -110 dBm | сигнал отсутствует или непригоден |

---

## Цветовая схема

Для Heatmap используется условная цветовая шкала:

| Уровень | Цвет |
|---|---|
| Excellent | красный |
| Good | оранжевый |
| Fair | жёлтый |
| Poor / Weak | синий |
| No Signal | не закрашивается |

---

## Формирование точек Heatmap

После получения JSON-пакета сервер извлекает:

- latitude;
- longitude;
- altitude;
- список сот;
- параметры сигнала.

На основе этих данных формируются точки измерений.

Пример структуры точки:

```cpp
struct HeatPoint {
    double lat;
    double lon;
    double value;
    int pci;
    int earfcn;
};
```

Поле `value` зависит от выбранного критерия:

- для RSRP используется `rsrp`;
- для RSRQ используется `rsrq`;
- для RSSI используется `rssi`;
- для Altitude используется `altitude`.

---

## Получение данных из PostgreSQL

Для построения Heatmap backend может использовать данные из базы.

Основные таблицы:

```text
telemetry_packets
telemetry_cells
```

Из таблицы `telemetry_packets` берутся:

- latitude;
- longitude;
- altitude;
- ts_client_ms.

Из таблицы `telemetry_cells` берутся:

- pci;
- earfcn;
- rsrp;
- rsrq;
- rssi;
- sinr.

Связь между таблицами выполняется через поле:

```text
telemetry_cells.packet_id
```

---

## Генерация изображения Heatmap

Heatmap может генерироваться двумя способами:

1. одной общей картинкой для всей области;
2. отдельной картинкой для каждого тайла.

В проекте используется генерация изображения, которое затем отображается поверх карты.

Алгоритм:

1. определяется область карты;
2. создаётся RGBA-буфер;
3. для каждого пикселя вычисляется координата;
4. ищутся ближайшие точки измерений;
5. выполняется IDW-интерполяция;
6. вычисленное значение переводится в цвет;
7. создаётся OpenGL texture;
8. texture отображается в окне карты.

---

## Сохранение Heatmap

Сгенерированные изображения сохраняются в директорию `build`.

Если Heatmap строится по тайлам, используется структура:

```text
build/
└─ <zoom>/
   └─ <x>/
      └─ <y>.png
```

Если Heatmap строится одной картинкой, она сохраняется в:

```text
build/heatmap.png
```

При повторном отображении уже сгенерированная картинка может быть загружена из файла без повторного вычисления.

---

## Отображение Heatmap в GUI

GUI реализован на основе:

- Dear ImGui;
- ImPlot;
- OpenGL.

Heatmap отображается поверх карты OpenStreetMap.

В интерфейсе добавлены элементы управления:

- выбор критерия:
  - RSRP;
  - RSRQ;
  - RSSI;
  - Altitude;

- выбор EARFCN;
- включение / выключение Heatmap;
- обновление Heatmap;
- отображение текущего количества точек.

---

## Работа с потоками

Генерация Heatmap может занимать заметное время, особенно при большом количестве точек.

Поэтому вычисления рекомендуется выполнять в отдельном потоке.

Это позволяет:

- не блокировать GUI;
- продолжать принимать телеметрию;
- обновлять карту во время вычислений;
- избежать зависания окна ImGui.

---

## Ограничения

Текущая реализация имеет следующие ограничения:

- точность Heatmap зависит от количества измеренных точек;
- при малом количестве данных карта может быть неполной;
- области без измерений не закрашиваются;
- генерация большой Heatmap может занимать время;
- цветовая шкала выбрана условно и может быть изменена.
